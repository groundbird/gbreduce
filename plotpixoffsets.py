...
infile = 'gb_pixinfo_20210730.txt'
...
moonlist = ['20210801_data_053023_swp_GB03', '20210801_data_053027_swp_GB04', '20210801_data_083133_swp_GB03', '20210801_data_083134_swp_GB04']
...
for file in todolist:
	...
	for line in infile:
		...
		# 		print(arr)
		# 		values.append(arr)
		if 'Difference in' in line:
			line = line.replace('Difference in phase is','').replace('in azimuth and',',').replace('in elevation.','').strip()
			line = line.split(',')
			print(i)
			if j % 4 == 3:
				...
				el.append(elval)
				# freq.append(freqval)
				outfile.write(str(az[-1]) + " " + str(el[-1]) + '\n')# + " " + str(freq[-1])+'\n')
				print(str(az[-1]) + " " + str(el[-1]) + '\n')# + " " + str(freq[-1])+'\n')
				# input('continue?')
			i+=1
			j+=1
	# if freq[0] > 6.0e9:
...
if docorr == False:
	...
	plt.xlim([-100,100])
	plt.ylim([-20,10])
	plt.savefig(outputdir+'pixel_positions_from_moon_zoom_comp.png')
